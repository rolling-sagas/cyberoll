...
export const onReportProblem = (story, router) => {
  ...
  openModal(
    <div className="bg-rs-background-2 flex flex-col rounded-2xl w-80 px-6 py-4 gap-3">
      ...
      <div className="text-base mb-2 text-center">
        Sign out of Rolling Sagas?
      </div>
      <Button className="rounded-xl" onClick={goLogout}>
        Sign out
      </Button>
      <Button className="rounded-xl" variant="outline" onClick={closeModal}>
        Cancel
      </Button>
    </div>
  );
