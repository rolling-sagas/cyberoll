 ...
 */
export default function usePageData(getData, size, itemsKey = 'items', totalKey = 'total') {
  ...
  const fetchData = useCallback(async (page, ...rest) => {
    setLoading(true);
    try {
  ...
  return [data, total, loading, page, hasMore, loadmore, setPageNum, reset];
}
