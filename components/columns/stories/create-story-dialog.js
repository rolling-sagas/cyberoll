...
export default function CreateStoryDialog({
  ...
  const canCreate =
    tName.trim().length > 0 && tDesc.trim().length > 0 && tImage.length > 0;
                ...
                <div className="pt-1 relative col-start-1 row-span-2">
                  <div className="w-9 h-9">
                    {tKeepPrivate ? (
                      ...
                      <ViewIcon className="text-rs-text-secondary" size={24} />
                    )}
                  </div>
                </div>
                <div className="font-semibold col-start-2 rows-start-1 mb-2">
                  Is public to everyone
                </div>
                <div className="flex items-center gap-2">
                  <Switch
                    checked={!tKeepPrivate}
                    onCheckedChange={(v) => setTKeepPrivate(!v)}
                    id="is-public-switch"
                  />
                </div>
              </div>
